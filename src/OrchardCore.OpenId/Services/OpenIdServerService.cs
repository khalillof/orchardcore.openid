...
public class OpenIdServerService : IOpenIdServerService
{
    ...
    public async Task<ImmutableArray<SecurityKey>> GetEncryptionKeysAsync()
    {
		...
		{
			return await GetPathCertificateKeys(settings.EncryptionCertificatePath);
		}
    ...
    public async Task<ImmutableArray<SecurityKey>> GetSigningKeysAsync()
    {
		...
		// If a certificate was explicitly provided, return it immediately
		// instead of using the fallback managed certificates logic.
		if (!string.IsNullOrEmpty(settings.SigningCertificatePath))
		{
			return await GetPathCertificateKeys(settings.SigningCertificatePath);
		}
	...
	private async Task<ImmutableArray<SecurityKey>> GetPathCertificateKeys(string path)
	{
		// If a certificate was explicitly provided, return it immediately
		// instead of using the fallback managed certificates logic.
		...
		if (!File.Exists(path))
		{
			_logger.LogWarning("The certificate Path : {path} - NOT FOUND", path);

			throw new NullReferenceException($"The certificate Path : {path} - NOT FOUND");
		}

		var Cert = await GetCertificateAsync(path, false);
		...
		{
			_logger.LogWarning("The certificate could not be found in the {path}", path);

			throw new NullReferenceException($"The certificate could not be found in the {path}");
		}
	}
	private async Task<string> GetPasswordAsync(string path, bool passprotected = true)
	{
		ArgumentNullException.ThrowIfNullOrEmpty(path);
		if (File.Exists(path))
		{
			...
			var pass = await reader.ReadToEndAsync();
			return passprotected ? _dataProtector.Unprotect(pass) : pass;
		}
		throw new ArgumentNullException(nameof(path));

	}

	private async Task<X509Certificate2> GetCertificateAsync(string path, bool passprotected = true)
	{
		// Extract the certificate password from the separate .pwd file.
		var password = await GetPasswordAsync(Path.ChangeExtension(path, ".pwd"), passprotected);
